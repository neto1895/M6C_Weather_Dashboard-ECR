...
function geoCODE_call(queryURL){
    ...
    fetch(queryURL)     
    // Fetch the data from the API using the query URL
        .then(function (response) {
            return response.json();
        })
        .then(function(data) { 
            // Extract the latitude and longitude from the API response data
            city = data[0].name;
            latitude = data[0].lat;
    ...
    function secondAPIcal(){ 
        // Function to retrieve and display the current weather data
        ...
        fetch(QueryURL_current)
            ...
            .then(function(data2){         
                // Display the retrieved weather data on the dashboard
